import { Component, OnInit, OnDestroy, HostListener, ElementRef, ViewChild, AfterViewInit, signal } from '@angular/core';
...
export class ClientLandingComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < N; i++) {
        for (let j = i + 1; j < N; j++) {
          ...
          if (dist < 130) {
            ctx.beginPath();
            ctx.strokeStyle = `rgba(99,102,241,${0.09 * (1 - dist / 130)})`;
            ctx.lineWidth   = 0.6;
  ...
  private initDashboardTilt(): void {
    ...
    dash.addEventListener('mouseleave', () => {
      dash.style.transform = 'perspective(1200px) rotateY(-6deg) rotateX(4deg)';
    });
  }
